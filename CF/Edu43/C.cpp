...
void read() {
	...
	// for (auto elem: arr) {
	// 	cerr << elem.fi.fi << " " << elem.fi.se << " " << elem.se << endl;
	// }
}
...
void solve() {
	...
	REP(i, n) {
		if (!pq.empty() && pq.top().fi >= arr[i].fi.se) {
			cout << arr[i].se << " " << pq.top().se << endl;
			return;
		}
		pq.push({arr[i].fi.se, arr[i].se});
	}
	cout << -1 << " " << -1 << endl;
}
